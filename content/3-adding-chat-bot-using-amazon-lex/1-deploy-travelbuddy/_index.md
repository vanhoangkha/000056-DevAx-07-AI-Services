...
7. Khi Change Set thực thi xong, các API HotelSpecials và FlightSpecials đã được triển khai.
8. Xác nhận các API đã được triển khai bằng cách kiểm tra việc gọi API bằng các URL được cung cấp trong **prod** stage
...
![API Gateway](/images/3/2.png?width=90pc)![API Gateway](/images/3/3.png?width=90pc)
Hãy đảm bảo rằng cả 2 microservice đều không có lỗi và hoạt động tốt.

#### Với microservice TripSearch thực hiện những bước sau:
9. Tải mã nguồn từ tập tin **TripSearch** và giải nén.
10. Trong terminal, *cd* tới thư mục đã giải nén và sử dụng câu lệnh sau để build:
```
mvn package shade:shade
```
11. Sử dụng AWS CLI để đóng gói hàm Lambda:
```
aws cloudformation package --template template.yml --s3-bucket <S3BucketLambdaCode> --output-template template-export.yml --profile aws-lab-env  --region us-east-1
```
Thay thế giá trị của <S3BucketLambdaCode>  và lưu tập tin.
12. Sử dụng AWS CLI để tạo và triển khai CloudFormation Change Set:
```
...
```
13. Khi ChangeSet được thực thi xong, TripSearch API dã được triển khai. Xác nhận API đã hoạt động chính xác bằng cách mở API Gateway, kiểm tra các lệnh gọi API bằng cách truy cập URL được cung cấp **prod** stage, tương tự như phần trước.
...
14. Mở **API Gateway** console và chọn **iDevelop - Flight Specials API**
15. Chọn **Stages**
16. Chọn **prod**
...
24. Lưu tập tin lại.
25. Ở đầu bài, bạn đã tải và giải nén tập tin **TravelBuddy.zip**. Sao chép và thay thế tập tin *apigClientxxxxx.js* trong thư mục */TravelBuddy/api/*
